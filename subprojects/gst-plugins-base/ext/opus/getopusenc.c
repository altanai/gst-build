...
#define DEFAULT_DRED_DURATION   100
...
gst_opus_enc_class_init (GstOpusEncClass * klass)
{
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass),
      PROP_PACKET_LOSS_PERCENT, g_param_spec_int ("packet-loss-percentage",
          ...
          GST_PARAM_MUTABLE_PLAYING));
  g_object_class_install_property (G_OBJECT_CLASS (klass),
    PROP_DRED_DURATION, g_param_spec_int ("dred-duration",
        "Dred duration", "Dred duration",
        DEFAULT_DRED_DURATION,
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
        GST_PARAM_MUTABLE_PLAYING));
  g_object_class_install_property (G_OBJECT_CLASS (klass),
...
gst_opus_enc_init (GstOpusEnc * enc)
{
  ...
  enc->dred_duration = DEFAULT_DRED_DURATION;
}
...
// Added 20% loss by Altanai 
  opus_multistream_encoder_ctl (enc->state,
      OPUS_SET_PACKET_LOSS_PERC (enc->packet_loss_percentage), 20);

// DRED added by Altanai, duration set to 1s
  opus_multistream_encoder_ctl (enc->state,
      OPUS_SET_DRED_DURATION (enc->dred_duration), 100);
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PACKET_LOSS_PERCENT:
      ...
      break;
    case PROP_DRED_DURATION:
      g_value_set_int(value, enc->dred_duration);
      break;
...
#define GST_OPUS_UPDATE_PROPERTY(prop,type,ctl) do { \
  ...
  switch (prop_id) {
    ...
    case PROP_PACKET_LOSS_PERCENT:
      ...
      break;
    case PROP_DRED_DURATION:
      GST_OPUS_UPDATE_PROPERTY (dred_duration, int, DRED_DURATION);
      break;
    case PROP_MAX_PAYLOAD_SIZE:
      g_mutex_lock (&enc->property_lock);
